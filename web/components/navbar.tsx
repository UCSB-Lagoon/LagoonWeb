...
import { Trophy, Target, Map, User, Vote } from "lucide-react";
...
export function Navbar() {
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-cream-50/80 border-b border-cream-200">
      <nav className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 font-display font-bold text-xl tracking-tight">
          <span className="text-orange-500">Lagoon</span>
        ...
        <ul className="flex items-center gap-1 text-sm text-ink-500">
          {links.map(({ href, label, icon: Icon }) => (
            <li key={href}>
              <Link
                href={href}
                className="flex items-center gap-1.5 px-3 py-2 rounded-full hover:bg-cream-100 hover:text-ink-900 transition"
              >
